...
private:
	...
	int countEvens(Node* temp)
	{
		if (temp)
		{
			if (temp->data % 2 == 0)
				return 1 + countEvens(temp->next);
			else
				return countEvens(temp->next);
		}
		else
			return 0;
	}
public:
	...
	int removeAll(int);
	int removerAlternates();
	void insertTillN(int n);
	void unionLists(LinkedList& list1, LinkedList& list2);
	int countEvens();
